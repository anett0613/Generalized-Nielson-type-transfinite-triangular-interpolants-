...
std::ostream& cagd::operator <<(std::ostream& lhs, const TriangulatedMesh3& rhs) {
	...
	for (vector<DCoordinate3>::const_iterator vit = rhs._vertex.begin();
		vit != rhs._vertex.end();
		++vit) {
		lhs << *vit << std::endl;
	}
    ...
    for (GLuint i=0; i<rhs.VertexCount();i++) {
        lhs << rhs._normal[i] << std::endl;
	}
    lhs<<std::endl;

    for (GLuint i=0; i<rhs.VertexCount();i++) {
        lhs << rhs._tex[i] << std::endl;
    }
    lhs<<std::endl;

	for (vector<TriangularFace>::const_iterator fit = rhs._face.begin();
		fit != rhs._face.end();
		++fit) {
        lhs << *fit << std::endl;
	}
    lhs << rhs._leftmost_vertex<< std::endl;
    lhs << rhs._rightmost_vertex<< std::endl;
    lhs<<std::endl;
    return lhs;
}
